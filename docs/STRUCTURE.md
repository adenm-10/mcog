...
| Untrained floors, all arms of Sweeps C and D | `tools/make_board_v2_floor.sh` | works; 6 cells, ~4 min. Sources `slurm/pins_board_v2.sh` and `slurm/pins_gamma_ladder.sh` rather than retyping either -- the one floor builder that survives. Its four predecessors (`make_v3{2,4,5}_floor.sh`, `make_v34_recontact_floor.sh`) were deleted 2026-09-08: each held its own retyped protocol string, which is the defect that produced digest `1a72f6438f34` against the sweep's `249434216cd2`. Their protocols survive where the project's own rule puts them -- `PROTOCOL.md` beside their numbers in `logs/eval/<name>/` -- and in git history |
...
| Result figures for a scored sweep | `tools/figs.py {v34\|sweepB} [out]` | works; one file since 2026-09-08 -- `figs_sweepB.py` already imported the palette, `load` and `one_digest` from `figs_v34.py`, so the shared library was living inside a script named after a superseded sweep. REFUSES to plot a panel whose cells disagree on `env_digest`, and asserts every panel drew artists and carries a title. Local disk only |
...
| Probe edge success across horizons | `tests/probe_edges.py` | works; 8 budgets on disk. **NOT a deletion path** -- `tests/summarize_horizon_sweep.py:29` imports it and `docs/TODO.md` F1 ("Highest value") names reading its output as the next action for the second budget rung. The old label was checked and corrected 2026-09-08 |
...
|---|---|---|
| The contact world: pymunk substrate + the obs/step contract over it | `domains/contact/world.py` | works; `planar_fingertips.py` and `physics.py` merged here 2026-09-08, the seam kept as a section rather than a file. TOP half is the only pymunk importer in the repo (true, and now GATED -- `cmd_layering` only ever checked that `option_graph/` was pymunk-free, so the isolation the docs claimed was unenforced). Also `face_frame`, `_tangential_speed`, `ContactFrameCommand`. BOTTOM half: `obs()` translation-invariant to ~1e-6, and **`ObsScales` is the ONE place any observation divisor may live** (a `static` gate forbids a bare divisor in `obs()`); `.v1()` reproduces the historical constants bug-for-bug, `.v2()` fixes them. A future substrate (a MuJoCo arm) replaces the top half and reuses the bottom |
