...
def main() -> None:
    """python tools/figs.py {v34|sweepB} [out_dir]"""
    which = sys.argv[1] if len(sys.argv) > 1 else "v34"
    default = {"v34": "media/v34", "sweepB": "media/sweepB"}
    if which not in default:
        raise SystemExit(f"usage: figs.py {{{'|'.join(default)}}} [out_dir]")
    out = sys.argv[2] if len(sys.argv) > 2 else default[which]
    os.makedirs(out, exist_ok=True)
    made = ([push_figure(out), recon_figure(out)] if which == "v34"
            else [sweepb_figure(out)])
    for p in made:
        print("wrote", p)
